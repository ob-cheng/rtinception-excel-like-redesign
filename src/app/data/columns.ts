...
export const columns: Column[] = [
  ...
  { key: "atpProduct",           label: "ATP / Key Product",                        width: 120 },
  ...
  { key: "totalCost",            label: "Total Budget",                             width: 120 },
  { key: "total2027Cost",        label: "2027 Spend",                               width: 120 },
  ...
  { key: "sampleSize",           label: "Sample Size",                              width: 120 },
...
export const VIEW_KEYS: Record<ViewKey, (keyof Idea)[]> = {
  ...
  // like `uid` so the spine appears once).
  "Funded": [
    ...FRANCHISE_KEYS,
...
// the grid's edge, then stick in place while the rest keeps scrolling. Each view pins its own
// contiguous block: Franchise/Funded pin the ranking block; Evidence pins the Franchise-owned
// framing (the read-only reference columns) so it stays in view while the endpoint/study columns
// scroll underneath. Keep every block contiguous within its view or the sticky offsets drift.
const FROZEN_KEYS_BY_VIEW: Record<ViewKey, (keyof Idea)[]> = {
  ...
  "Funded": ["brandRanking", "areaPrioritization", "pathway"],
};
...
export const EVIDENCE_READONLY_KEYS: (keyof Idea)[] = ["pathway", "atpProduct", "project", "researchQuestions", "potentialClaims"];
...
const READONLY_BY_VIEW: Partial<Record<ViewKey, (keyof Idea)[]>> = {
  ...
  "Franchise": ["pos", "region", "totalCost", "total2027Cost"],
};
